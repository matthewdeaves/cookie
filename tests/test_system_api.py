...
class TestResetDatabase:
    ...
    def test_reset_deletes_all_user_data(self, mock_call_command, client, populated_database):
        ...
        from apps.profiles.models import Profile
        from apps.recipes.models import (
            Recipe, RecipeFavorite, RecipeCollection,
            RecipeCollectionItem, RecipeViewHistory,
            ServingAdjustment, CachedSearchImage,
        )
        from apps.ai.models import AIDiscoverySuggestion
    ...
    def test_reset_preserves_search_sources(self, mock_call_command, client, db):
        ...
        initial_count = SearchSource.objects.count()
        ...
        # Verify sources still exist (count should be same or greater after re-seeding)
    ...
    def test_reset_preserves_ai_prompts(self, mock_call_command, client, db):
        ...
        initial_count = AIPrompt.objects.count()
        ...
        # Verify prompts still exist (count should be same or greater after re-seeding)
    ...
    def test_reset_clears_image_directories(
        ...
        # Should have removed both image directories
        recipe_images_dir = os.path.join(settings.MEDIA_ROOT, 'recipe_images')
        search_images_dir = os.path.join(settings.MEDIA_ROOT, 'search_images')

        # Check rmtree was called for both directories
    ...
    def test_reset_runs_migrations(self, mock_call_command, client, db):
        ...
        migrate_calls = [
            call for call in mock_call_command.call_args_list
            if call[0][0] == 'migrate'
        ]
