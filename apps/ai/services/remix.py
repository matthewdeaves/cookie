...
def create_remix(
    ...
    thread = threading.Thread(target=_generate_tips_background, args=(remix.id,), daemon=True)
    thread.start()
...
def estimate_nutrition(
    ...
    for key, value in original.nutrition.items():
        ...
        readable_key = "".join(" " + c if c.isupper() else c for c in readable_key).strip().title()
