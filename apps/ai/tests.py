...
class AIPromptModelTests(TestCase):
    ...
    def test_format_user_prompt(self):
        ...
        formatted = prompt.format_user_prompt(instruction="Simmer for 20 minutes", duration="20 minutes")
...
class AIResponseValidatorTests(TestCase):
    ...
    def test_validate_selector_repair_valid(self):
        ...
        response = {"suggestions": [".recipe-title", "h1.recipe-name"], "confidence": 0.85}
...
class AIAPITests(TestCase):
    ...
    def test_update_prompt_endpoint(self):
        ...
        response = self.client.put(
            "/api/ai/prompts/recipe_remix", data={"model": "openai/gpt-4o"}, content_type="application/json"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_prompt_invalid_model(self, mock_service_class):
        ...
        response = self.client.put(
            "/api/ai/prompts/recipe_remix", data={"model": "invalid/model-name"}, content_type="application/json"
        )
...
class TimerNamingAPITests(TestCase):
    ...
    def test_timer_name_endpoint_missing_step_text(self):
        ...
        response = self.client.post(
            "/api/ai/timer-name", data={"step_text": "", "duration_minutes": 10}, content_type="application/json"
        )
...
class SelectorRepairServiceTests(TestCase):
    ...
    def test_repair_selector_low_confidence_no_update(self, mock_service_class):
        ...
        mock_service = MagicMock()
        mock_service.complete.return_value = {"suggestions": [".maybe-selector"], "confidence": 0.5}
        mock_service_class.return_value = mock_service
    ...
    def test_repair_selector_auto_update_disabled(self, mock_service_class):
        ...
        mock_service = MagicMock()
        mock_service.complete.return_value = {"suggestions": [".new-selector"], "confidence": 0.95}
        mock_service_class.return_value = mock_service
    ...
    def test_repair_selector_truncates_html(self, mock_service_class):
        ...
        mock_service = MagicMock()
        mock_service.complete.return_value = {"suggestions": [".selector"], "confidence": 0.8}
        mock_service_class.return_value = mock_service
...
class AIFeatureFallbackTests(TestCase):
    ...
    def test_tips_returns_503_with_action_field_when_ai_unavailable(self, mock_generate):
        ...
        response = self.client.post("/api/ai/tips", data={"recipe_id": self.recipe.id}, content_type="application/json")
    ...
    def test_remix_suggestions_returns_503_when_ai_unavailable(self, mock_suggestions):
        ...
        response = self.client.post(
            "/api/ai/remix-suggestions", data={"recipe_id": self.recipe.id}, content_type="application/json"
        )
